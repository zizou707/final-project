...
function UserChat({chat,user}) {
  ...
  const { onlineUsers,notifications, markThisUserNotificationsAsRead } = useContext(messagesContext)
  ...
  return (
    ...
    onClick={() =>{ if (thisUserNotifications?.length !==0){
       ...
       <div className="d-flex ">
         ...
         <div className="text-content">
           ...
           <div className="text"> {latestMessage?.messageText && (<span> {truncateText(latestMessage?.messageText)} </span>) } </div>
         </div>
       </div>
       <div className="d-flex flex-col items-end ">
             <div className="date">{moment(latestMessage?.createdAt).calendar()}</div>
             <span className={thisUserNotifications?.length > 0 ? "this-user notification-count" : ""} >
              <span> {thisUserNotifications?.length > 0 ? thisUserNotifications?.length : ""} </span>
            </span>
