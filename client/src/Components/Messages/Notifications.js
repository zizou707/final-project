...
function Notifications() {
    ...
    const { notifications ,markAllNotificationsAsRead, markNotificationAsRead } = useContext(messagesContext);
    const scroll = useRef();
    ...
    <div className="notifications">
     ...
     {isOpen ? (<div  className="notifications-box">
        <div className="notifications-header ">
            <h3>Notificaions</h3>
            <div className="mark-as-read" onClick={()=>{markAllNotificationsAsRead(notifications);setIsOpen(false)}}>
              mark all as read
            </div>
        </div>
        {modifiedNotifications?.length === 0 ? <span>No notifiactions yet...</span> : null}
      <div ref={scroll}  id='modified-notifications' >
        {modifiedNotifications && modifiedNotifications.map((n,index)=>{
          return (
            <div onClick={()=>{markNotificationAsRead(n,userChats,user,notifications);setIsOpen(false)}} key={index} className={n.isRead ? "notification" : "notification not-read"}>
